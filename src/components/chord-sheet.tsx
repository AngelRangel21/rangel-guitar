...
const isSectionMarker = (text: string): boolean => {
  ...
  const content = text.substring(1, text.length - 1).toLowerCase()
      ...
      {text.split('\n').map((line, index) => {
        ...
        return (
          <div key={key} className='whitespace-pre'>
            {tokens.map((token, i) => {
              const chord = `chord-${i}-${token}`
              if (token.type === 'chord') {
                return (
                  <Chord key={chord} name={token.value} transpose={transpose} />
                )
              }
              // Si el token es una sección, lo renderiza en negrita. ej: [Intro], [Verso], etc.
              if (token.type === 'section') {
                return <strong className='font-bold'>{token.value}</strong>
              }

              return (
                <React.Fragment key={token.value}>{token.value}</React.Fragment>
              )
            })}
          </div>
        )
