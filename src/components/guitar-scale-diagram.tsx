 ...
 */
export function GuitarScaleDiagram({
  scaleNotes,
  rootNote,
  className
}: GuitarScaleDiagramProps) {
  ...
  return (
    <div
      className={cn(
        'overflow-x-auto p-2 bg-background/50 rounded-lg border',
        className
      )}
    >
      <svg
      ...
      >
        <g transform='translate(30, 25)' className='text-foreground'>
          ...
          {Array.from({ length: FRET_COUNT + 1 }).map((_, i) => {
            ...
            return (
              <line
                key={`fret-v-line-${fretPos}`}
                x1={fretPos * FRET_WIDTH + FRET_WIDTH / 2}
                y1={0}
                x2={fretPos * FRET_WIDTH + FRET_WIDTH / 2}
                y2={STRING_SPACING * (STRING_COUNT - 1)}
                stroke='hsl(var(--muted-foreground))'
                strokeWidth={fretPos === 0 ? 5 : 1} // Cejuela más gruesa.
              />
            )
          })}
          ...
          {Array.from({ length: STRING_COUNT }).map((_, i) => {
            ...
            return (
              <line
                key={`string-h-line-${fretPos}`}
                x1={FRET_WIDTH / 2}
                y1={fretPos * STRING_SPACING}
                x2={FRET_WIDTH * FRET_COUNT + FRET_WIDTH / 2}
                y2={fretPos * STRING_SPACING}
                stroke='currentColor'
                strokeWidth={1 + fretPos * 0.15} // Grosor creciente para simular cuerdas reales.
              />
            )
          })}
          ...
          {dots.map(({ string, fret, isRoot, noteName }) => {
            const cx = fret === 0 ? 0 : fret * FRET_WIDTH
            const cy = string * STRING_SPACING
            const noteKey = `note-${string}-${fret}-${noteName}`
            ...
            return (
              <g key={noteKey}>
                <circle
                  ...
                  r={DOT_RADIUS}
                  fill={
                    isRoot ? 'hsl(var(--accent))' : 'hsl(var(--foreground))'
                  }
                  stroke={
                    isRoot ? 'hsl(var(--accent))' : 'hsl(var(--foreground))'
                  }
