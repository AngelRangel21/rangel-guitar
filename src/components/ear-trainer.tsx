...
export function EarTrainer() {
  ...
  const [gameState, setGameState] = useState<
    'level-select' | 'playing' | 'level-complete'
  >('level-select')
  ...
  const handleGuess = (guess: string) => {
    if (gameState !== 'playing' || feedback || selectedLevelIndex === null)
      return
    ...
    if (guess === currentNote) {
      ...
      if (newScore >= level.notesToGuess) {
        ...
        if (
          nextLevelIndex > unlockedLevelIndex &&
          nextLevelIndex < flattenedLevels.length
        ) {
  ...
  const level =
    selectedLevelIndex !== null ? flattenedLevels[selectedLevelIndex] : null
    ...
    <Card className='w-full max-w-md mx-auto'>
      ...
      <CardContent className='flex flex-col items-center gap-6'>
        ...
        <div className='h-12 flex items-center justify-center'>
          {feedback === 'correct' && (
            <CheckCircle className='h-10 w-10 text-green-500 animate-content-in' />
          )}
          {feedback === 'incorrect' && (
            <XCircle className='h-10 w-10 text-destructive animate-content-in' />
          )}
        </div>
        ...
        {gameState === 'level-complete' && (
          <div className='text-center p-4 rounded-lg bg-green-500/10 border border-green-500/30'>
            ...
            selectedLevelIndex < flattenedLevels.length - 1 ? (
              ...
              <Button onClick={handleBackToMenu}>{t('backToLevels')}</Button>
            )}
          </div>
        )}
      </CardContent>
